    ...
    public static class RandomExtensions
    {
        ...
        public static void Shuffle<T>(this List<T> list)
        {
            for (int i = 1; i < list.Count; i++)
            {
                ...
                (list[i], list[indRnd]) = (list[indRnd], list[i]);
            }
        }
        ...
        public static void Shuffle<T>(T[] array)
        {
            ...
            {
                int indRnd = Random.Range(0, i + 1);
                (array[i], array[indRnd]) = (array[indRnd], array[i]);
            }
        }
        ...
        {
            float t = Random.Range(0f, 1f);
            return new Vector3(Mathf.Lerp(point1.x, point2.x, t), Mathf.Lerp(point1.y, point2.y, t),
                Mathf.Lerp(point1.z, point2.z, t));
        }
        ...
        public static int RandomSeedByString(string seedString)
        {
            var seed = string.IsNullOrEmpty(seedString) 
                ? RandomString(8).GetHashCode() 
                : seedString.GetHashCode();
            return seed;
        }
